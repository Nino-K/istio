...
var (
	scope      = log.Scope
	collection = metadata.SyntheticServiceEntry.Collection
...
func init() {
	...
	b.RegisterInfo(metadata.Pod)
	b.RegisterInfo(metadata.Node)
	b.RegisterInfo(metadata.Service)
	b.RegisterInfo(metadata.Endpoints)
	Schema = b.Build()
}
...
type Handler struct {
	...
	localityStrategy locality.Strategy
...
func NewHandler(domainSuffix string, listener processing.Listener) *Handler {
	...
	return &Handler{
		...
		localityStrategy: locality.NewStrategy(pods, nodes),
...
func (p *Handler) Handle(event resource.Event) {
	...
	case metadata.Endpoints.Collection:
		// Update the projections
		p.handleEndpointsEvent(event)
	case metadata.Service.Collection:
		// Update the projections
		p.handleServiceEvent(event)
	case metadata.Node.Collection:
		// Just add the node to the cache.
		p.nodesHandler.Handle(event)
	case metadata.Pod.Collection:
...
func (p *Handler) toMcpResource(svcEntry resource.Entry, endpoints *coreV1.Endpoints) (*mcp.Resource, bool) {
	...
	se := convert.Service(svc, svcMeta, svcEntry.ID, p.domainSuffix)
